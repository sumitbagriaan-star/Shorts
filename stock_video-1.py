...
def fetch_background_video(query, filename="background.mp4"):
    ...
    for i in range(attempts):
        ...
        try:
            _download_file(chosen["link"], path)
            size = os.path.getsize(path)
            if size < 100000:
                raise ValueError("Downloaded file too small (" + str(size) + " bytes) - likely broken")
            print("Background video downloaded:", path, "(", size, "bytes )")
            return path
        except Exception as e:
            last_error = e
            print("Attempt", i + 1, "failed:", e, "- trying another clip...")
            if os.path.exists(path):
                os.remove(path)

    raise RuntimeError("Could not download a valid background video after " + str(attempts) + " attempts: " + str(last_error))
