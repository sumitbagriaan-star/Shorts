...
def pick_background_source(topic):
    """
    50% chance: use one of the user's uploaded photos (if any exist) with a
    Ken Burns zoom effect. Otherwise: fetch a matching stock video from Pexels.
    """
    photo_files = []
...
def run():
    ...
    if video_link:
        ...
        try:
            ...
            inspired_topic = analysis["suggested_topic"]
            print("-> Using this as the background search topic:", inspired_topic)
        except Exception as e:
            print("Could not analyze the link, falling back to a random topic. Reason:", e)
    ...
    bg_topic = inspired_topic or data["topic"]
